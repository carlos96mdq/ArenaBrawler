...
#include "AbilityTask_PlayMontageAndWaitEv.generated.h"

//DECLARE_DYNAMIC_MULTICAST_DELEGATE(FMontageWaitEvSimpleDelegate);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FMontageWaitEventSimpleDelegate);
 ...
 */
UCLASS()
class ARENABRAWLER_API UAbilityTask_PlayMontageAndWaitEv : public UAbilityTask
{
	GENERATED_BODY()
public:

	UPROPERTY(BlueprintAssignable)
	FMontageWaitEventSimpleDelegate	OnCompleted;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitEventSimpleDelegate	OnBlendedIn;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitEventSimpleDelegate	OnBlendOut;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitEventSimpleDelegate	OnInterrupted;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitEventSimpleDelegate	OnCancelled;

	UPROPERTY(BlueprintAssignable)
	FMontageWaitEventDelegate OnMontageEvent;
	...
	/**
	 * Start playing an animation montage on the avatar actor and send all the montage notifications 
	...
	static UAbilityTask_PlayMontageAndWaitEv* CreatePlayMontageAndWaitEvProxy(UGameplayAbility* OwningAbility, FName TaskInstanceName, UAnimMontage* MontageToPlay, float Rate = 1.f, FName StartSection = NAME_None, bool bStopWhenAbilityEnds = true, float AnimRootMotionTranslationScale = 1.f, float StartTimeSeconds = 0.f, bool bAllowInterruptAfterBlendOut = false);
